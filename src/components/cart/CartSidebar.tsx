...
export function CartSidebar() {
  ...
  const handleRemove = (productId: string, variantOptions?: Record<string, string>) => {
                ...
                >
                  <AnimatePresence>
                    {items.map((item, index) => {
                      const itemPrice = item.variant?.price ? getPriceAmount(item.variant.price) : getPriceAmount(item.product.price);
                      const itemTotal = itemPrice * item.quantity;
                      const id = `${item.product.id}-${JSON.stringify(item.variant?.options)}-${index}`;
                      ...
                      return (
                        ...
                        >
                          ...
                          >
                            <Image
                              src={item.product.thumbnail || 'https://images.unsplash.com/photo-1505740420928-5e560c06d30e?w=200'}
                            ...
                            <div className="flex items-end justify-between mt-2">
                              <div className="flex items-center gap-2">
                                ...
                                <motion.button
                                  ...
                                  whileTap={{ scale: 0.9 }}
                                  onClick={() => handleRemove(item.product.id, item.variant?.options)}
