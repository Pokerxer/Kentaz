...
const categories = [
  ...
  { name: 'Kiddies', handle: 'kiddies-fashion' },
  { name: 'Skincare', handle: 'skincare' },
  { name: 'Luxury Hair', handle: 'human-hair' },
...
interface Product {
  ...
  variants?: { id?: string; title?: string; options?: Record<string, string>; prices?: { amount: number }[]; inventory_quantity?: number }[];
...
function ProductsPage() {
  ...
  useEffect(() => {
    fetch('http://localhost:9000/api/store/products')
      .then(res => res.json())
      .then(data => {
  ...
  const categoryCounts = useMemo(() => {
    ...
    products.forEach(p => {
      const handle = p.collection?.handle || 'other';
  ...
  const filteredAndSortedProducts = useMemo(() => {
    ...
    if (activeCategory !== 'all') {
      result = result.filter(p => p.collection?.handle === activeCategory);
    }
    ...
    if (selectedColors.length > 0) {
      result = result.filter(p => 
        ...
        p.options?.some(o => o.name.toLowerCase() === 'color' && o.values.some(v => selectedColors.includes(v.toLowerCase())))
      );
    }
    ...
    if (selectedSizes.length > 0) {
      result = result.filter(p =>
        ...
        p.options?.some(o => o.name.toLowerCase() === 'size' && o.values.some(v => selectedSizes.includes(v)))
      );
    }
    ...
    if (inStockOnly) {
      result = result.filter(p => 
        p.variants?.some(v => (v.inventory_quantity || 0) > 0) || !p.variants
      );
    }
    ...
    switch (sortBy) {
      ...
      case 'price_desc':
        ...
        break;
      case 'bestselling':
        result.sort((a, b) => (b.tags?.some(t => t.value === 'bestseller') ? 1 : 0) - (a.tags?.some(t => t.value === 'bestseller') ? 1 : 0));
        break;
      case 'newest':
        result.sort((a, b) => new Date(b.created_at || 0).getTime() - new Date(a.created_at || 0).getTime());
        break;
      default:
        result.sort((a, b) => (b.tags?.some(t => t.value === 'featured') ? 1 : 0) - (a.tags?.some(t => t.value === 'featured') ? 1 : 0));
    }
